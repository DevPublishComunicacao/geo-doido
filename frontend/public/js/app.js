...
function lerConfigStorage() {
  const stored = sessionStorage.getItem('where-config');
  configPais = null;
  configModo = 'mundo';
  ...
  if (stored) {
    ...
    sessionStorage.removeItem('where-config');
  } else {
    configTempoRodada = 0;
    configRolezinho = true;
  }
...
function getUsuarioLogado() {
  try {
    return JSON.parse(localStorage.getItem('where-usuario') || 'null');
  } catch(e) { return null; }
}
